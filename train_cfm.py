...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # RoPE on centers for position awareness
        cos, sin = self.rotary(seqlen, x.device, centers.dtype)
        centers = apply_rotary_emb(centers, cos, sin)

        # Expand metric/v for GQA: (B, n_kv, T, d_h) -> (B, n_heads, T, d_h)
        if self.num_kv_heads != self.num_heads:
            metric = metric.unsqueeze(2).expand(-1, -1, self.heads_per_group, -1, -1)
            metric = metric.reshape(bsz, self.num_heads, seqlen, d_h)
